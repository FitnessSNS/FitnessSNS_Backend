...
exports.createUserRunning = async (email, longitude, latitude) => {
    try {
        ...
        // 운동 위치 레코드 생성
        const running = await prisma.ExerciseLocation.create({
            data: {
                user_id: user[0].id,
                longitude: longitude,
                latitude: latitude
        ...
        const startTime = `${running.created_at.getHours()}:${running.created_at.getMinutes()}:${running.created_at.getSeconds()}`;
...
exports.createUserRunningCheck = async (email, longitude, latitude) => {
    try {
        ...
        // 기존에 생성한 운동 기록이 있을 경우
        let addDistance, addTime, addCalorie;
        let exerciseChange;
        let forceEnd = false;
        ...
        // 운동 기록이 없을 경우 새로 생성
            ...
            addTime = new Date(timeDiff);
            addCalorie = Number(calorie);
            
            exerciseChange = await prisma.Exercise.create({
                data: {
                    user_id: user[0].id,
                    distance: addDistance,
                    time: addTime,
